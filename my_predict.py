...
if __name__ == "__main__":
    mode = "batch_predict"
    model_path = 'weights/method_ulite/best_epoch_weights.pth'
    count = False
    name_classes = ["background", "1", "2"]
    img_size = [640,352]
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    batch_origin_path = "strawberry_dataset/test/images_prepped_test"  # 设置测试集图片的文件夹路径
    batch_save_path = "results/temp"  # 设置预测结果保存的路径


    #from nets.ulite import self_net
    #from nets.LightMUNet import LightMUNet
    #from nets.MALUNet import MALUNet
    from nets.ulite import self_net
    model = self_net(num_classes=3).eval().to(device)
    # from torchvision.models.segmentation import deeplabv3_resnet50
    # model = deeplabv3_resnet50(pretrained=False, num_classes=3, aux_loss=None).eval().to(device)
    #model = MALUNet().eval()

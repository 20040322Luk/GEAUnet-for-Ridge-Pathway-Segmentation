...
if __name__ == "__main__":
    # 训练好的模型的路径
    model_path = 'weights/method_ghpa_gab_ca_res/best_epoch_weights.pth'
    # from nets.ulite import self_net
    from nets.method_ghpa_gab_ca_res import self_net
    num_classes = 3  # 异常类型数
    # model = deeplabv3_resnet50(pretrained=False, num_classes=num_classes, aux_loss=None).eval()
    # model = self_net(num_classes=3).eval()
    model = self_net().eval()
    ...
    ### 计算 class IoU 分数 ####
    pred_dir = 'results/temp/npy'
    #base_dir = 'results/baseline_predictions/npy'
    base_dir = 'results/temp/npy'
    #gt_dir = 'results/ground_truths'
    gt_dir = 'strawberry_dataset/test/test_gt'

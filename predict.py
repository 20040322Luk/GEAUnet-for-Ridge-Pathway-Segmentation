...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------#
    #   batch_origin_path   指定了用于批量检测的图片的文件夹路径
    #   batch_save_path     指定了批量检测完图片的保存路径
    #
    #   batch_origin_path和batch_save_path仅在mode='batch_predict'时有效
    # -------------------------------------------------------------------------#
    batch_origin_path = "batch_img/"
    batch_save_path = "batch_img_out/"
    ...
    if mode == "dir_predict":
        ...
        for img_name in tqdm(img_names):
            ...
            if img_name.lower().endswith(
                ...
                r_image, pred_array = unet.detect_image(image, return_array=True)
                ...
                np.save(os.path.join(dir_save_path, os.path.splitext(img_name)[0] + '_result.npy'), pred_array)
    ...
    if mode == "batch_predict":
        ...
        for img_name in tqdm(img_names):
            ...
            if img_name.lower().endswith(
                ...
                r_image, pr = unet.detect_image(image, return_pr=True)  # 调整detect_image函数，使其返回pr
                ...
                np.save(os.path.join(npy_save_path, img_name.replace('.jpg', '.npy')), pr)

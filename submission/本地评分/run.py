...
if __name__ == "__main__":
    mode = "batch_predict"
    model_path = 'last_epoch_weights (2).pth'
    count = False
    name_classes = ["background", "1", "2", "3"]
    img_size = [224,224]
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    batch_origin_path = "./gt_imgs"  # 设置测试集图片的文件夹路径
    batch_save_path = "./temp"  # 设置预测结果保存的路径
    ...
    if mode == "batch_predict":
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                ...
                npy_save_path = os.path.join(batch_save_path)
                if not os.path.exists(npy_save_path):
                    os.makedirs(npy_save_path)
                np.save(os.path.join(npy_save_path, "c_prediction_" + img_name.replace('.jpg', '.npy')), pr)
